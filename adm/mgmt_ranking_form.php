...
if (!empty($db_obj->select_where_with_param("APND_FILE_M", "APND_FILE_SNO = 'top' and APND_FILE_CATE = 2 LIMIT 1"))) {
    $top_data = $db_obj->select_where_with_param("APND_FILE_M", "APND_FILE_SNO = 'top' and APND_FILE_CATE = 2 LIMIT 1");
    $top = $top_data[0];
}
if (!empty($db_obj->select_where_with_param("APND_FILE_M", "APND_FILE_SNO = 'mid' and APND_FILE_CATE = 2 LIMIT 1"))) {
    $mid_data = $db_obj->select_where_with_param("APND_FILE_M", "APND_FILE_SNO = 'mid' and APND_FILE_CATE = 2 LIMIT 1");
    $mid = $mid_data[0];
}
if (!empty($db_obj->select_where_with_param("APND_FILE_M", "APND_FILE_SNO = 'popup' and APND_FILE_CATE = 2 LIMIT 1"))) {
    $popup_data = $db_obj->select_where_with_param("APND_FILE_M", "APND_FILE_SNO = 'popup' and APND_FILE_CATE = 2 LIMIT 1");
    $popup = $popup_data[0];
}
if (!empty($db_obj->select_where_with_param("APND_FILE_M", "APND_FILE_SNO = 'bottom' and APND_FILE_CATE = 2 LIMIT 1"))) {
    $bottom_data = $db_obj->select_where_with_param("APND_FILE_M", "APND_FILE_SNO = 'bottom' and APND_FILE_CATE = 2 LIMIT 1");
    ...
    <main>
        ...
        <div class="row g-5">
            <div class="col-md-5 col-lg-12">
                <form name="uploadForm" id="uploadForm" method="post" action="mgmt_ranking_form_update.php" enctype="multipart/form-data">
                <!-- onsubmit="return formSubmit(this);" -->
                <input type="hidden" name="APND_FILE_CATE" value="2">
